...
function verifyXls(id){
	var x = document.getElementById(id);
	  ...
	  if(!patn.test(x.value)){	
		    document.getElementById(id).outerHTML="<input class=\"form-control\" type=\"file\"   id="+id+" name="+id+" datatype=\"*\"  nullmsg=\"请上传xls文件！\" onchange=\"verifyXls('"+id+"');\"/>";
	...
	function checkXls(id,className){
		  var x = document.getElementById(id);
		  if(!x || !x.value) return;
		  var patn = /\.XLS$|\.xls$|\.XLSX$|\.xlsx$/;	
		  if(!patn.test(x.value)){	
		    document.getElementById(id).outerHTML="<input class="+className+"  style=\"width: 400px;\"  type=\"file\" id="+id+" name="+id+"  onchange=\"checkXls('"+id+"');\"/>";
			alert("您选择的似乎不是xls文件。或者不是类型的文件");
		  }

		}
	
	
	function verifyTxt(id){
		var x = document.getElementById(id);
		  if(!x || !x.value) return;
		  var patn = /\.txt$|\.TXT$/;	
		  if(!patn.test(x.value)){	
			    document.getElementById(id).outerHTML="<input class=\"inp_l\" type=\"file\"   id="+id+" name="+id+" datatype=\"*\"  nullmsg=\"请上传txt文件！\" onchange=\"verifyTxt('"+id+"');\"/>";
			    alert("您选择的似乎不是txt文件！");
		  }
		
	}
	function verifyXml(id){
		var x = document.getElementById(id);
		  if(!x || !x.value) return;
		  var patn = /\.XML|\.xml$/;	
		  if(!patn.test(x.value)){	
			    document.getElementById(id).outerHTML="<input class=\"form-control\" type=\"file\"   id="+id+" name="+id+" datatype=\"*\"  nullmsg=\"请上传xml文件！\" onchange=\"verifyXml('"+id+"');\"/>";
			    alert("您选择的似乎不是xml文件！");
		  }
...
function verifyApk(id){
    var x = document.getElementById(id);
    if(!x || !x.value) return;
    var patn = /\.apk$|\.APK$/;
    if(!patn.test(x.value)){
        document.getElementById(id).outerHTML="<input class=\"inp_l\" type=\"file\"   id="+id+" name="+id+" datatype=\"*\"  nullmsg=\"请上传apk文件！\" onchange=\"verifyApk('"+id+"');\"/>";
        alert("您选择的似乎不是apk文件！");
    }

}
	
	function verifyUpload(id,className){
		  var x = document.getElementById(id);
		  if(!x || !x.value) return;
		  var patn = /\.RAR$|\.rar$|\.zip$|\.ZIP$|\.doc$|\.DOC$|\.docx$|\.DOCX$|\.xls$|\.XLS$|\.xlsx$|\.xlsx$|\.pdf$|\.PDF$|\.ppt$|\.PPT$|\.pptx$|\.PPTX$|\.txt$|\.TXT$|\.jpg$|\.JPG$|\.bmp$|\.BMP$|\.png$|\.PNG$|\.gif$|\.GIF$/;	
		  if(!patn.test(x.value)){	
		    document.getElementById(id).outerHTML="<input class=\"form-control\"   type=\"file\" id="+id+" name="+id+"  onchange=\"verifyUpload('"+id+"');\"/>";
