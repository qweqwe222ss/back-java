...
<body>
					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<form class="form-horizontal"
								action="<%=basePath%>normal/adminIpCountAction!list.action"
								...
								<div class="col-md-12 col-lg-4">
									<fieldset>
										<div class="control-group">
											<div class="controls">
												<!-- <s:textfield id="ip_para" name="ip_para"
													cssClass="form-control " placeholder="ip" /> -->
												<input id="ip_para" name="ip_para" class="form-control " placeholder="ip" value="${ip_para}" />
											</div>
										</div>
									</fieldset>
								</div>
							...
							<div class="col-md-12 col-lg-12">
								<div class="col-md-12 col-lg-3">
									<button type="button" class="btn btn-light btn-block"
										onclick="clearData();">清除所有请求数</button>
								</div>
							</div>
			...
			<div class="row">
				<div class="col-md-12">
					...
					<div class="panel panel-default">
						...
						<a href="javascript:batchAddBlack('${ip}')"
							class="btn btn-light" style="margin-bottom: 10px"><i
							class="fa fa-pencil"></i>批量添加黑名单</a>
							...
							<table class="table table-bordered table-striped">
								...
								<tbody>
									...
									<c:forEach items="${page.getElements()}" var="item" varStatus="stat">
										<tr>
											<td><a href="#" onClick="getUrlsCount('${item.ip}')">${item.ip}</a></td>
											<td>
												<c:if test="${item.type == 'black'}">
													<span class="right label label-danger">黑名单</span>
												</c:if> 
												<c:if test="${item.type == 'white'}">
											...
											<td>
												<%-- <fmt:parseDate value="${item.create_time}" pattern="yyyy-MM-dd'T'HH:mm:ss" var="myParseDate"></fmt:parseDate> 
											...
											<td>
												<div class="btn-group">
													...
													<ul class="dropdown-menu" role="menu">
														<li><a
															href="javascript:addBlack('${item.ip}')">加入黑名单</a></li>
														<li><a
															href="javascript:addLock('${item.ip}')">加入锁定名单</a></li>
													</ul>
												</div>
											</td>
										</tr>
	...
	<form action="<%=basePath%>normal/adminIpCountAction!addBlack.action"
		method="post" id="succeededForm">
		
		<!-- <s:hidden name="menu_ip" id="menu_ip"></s:hidden> -->
		<input type="hidden" name="menu_ip" id="menu_ip" value="${menu_ip}">
	...
	<form action="<%=basePath%>normal/adminIpCountAction!addLock.action"
		method="post" id="succeededForm">
		
		<!-- <s:hidden name="menu_ip" id="lock_menu_ip"></s:hidden> -->
		<input type="hidden" name="menu_ip" id="lock_menu_ip" value="${menu_ip}">
		<%-- 	<s:hidden name="query_symbol" id="query_symbol"></s:hidden> --%>
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<input id="limit_count" name="limit_count" class="limit_count"
										placeholder="请输入数字" value="${limit_count}">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-4">
									<input id="login_safeword" type="password" name="login_safeword" class="login_safeword"
										placeholder="请输入登录人资金密码" value="${login_safeword}">
								</div>
							</div>
	...
	<div class="form-group">
		<div class="col-sm-1">
			<!-- 模态框（Modal） -->
			<div class="modal fade" id="urls_form" tabindex="-1" role="dialog"
			...
			var url = "<%=basePath%>normal/adminIpCountAction!getUrlsCount.action";
				...
				goNewAjaxUrl(url, data, function(tmp) {
					...
					for (var i = 0; i < tmp.urls_count.length; i++) {
						str += '<tr>' + '<td>' + tmp.urls_count[i].url + '</td>'
