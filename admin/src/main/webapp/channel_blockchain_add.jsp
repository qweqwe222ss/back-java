...
<%-- 	<%@ include file="include/menu_left.jsp"%> --%>
					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
								...
								<form class="form-horizontal"
									action="<%=basePath%>normal/adminChannelBlockchainAction!add.action"
									...
									<div class="form-group">
										...
										<div class="col-sm-3">
											<!-- <s:select id="coin" cssClass="form-control " name="coin"
												list="coins_map" listKey="key" listValue="value"
												value="coin" /> -->
											<select id="coin" name="coin" class="form-control " >
												<c:forEach items="${coins_map}" var="item">
													<option value="${item.key}" <c:if test="${coin == item.key}">selected="true"</c:if> >${item.value}</option>
												</c:forEach>
											</select>*不可修改
									...
									<div class="form-group">
										...
										<div class="col-sm-3">
											<!-- <s:select id="blockchain_name" cssClass="form-control "
												name="blockchain_name" list="blockchain_name_map"
												listKey="key" listValue="value" value="blockchain_name" /> -->
											<select id="blockchain_name" name="blockchain_name" class="form-control " >
												<c:forEach items="${blockchain_name_map}" var="item">
													<option value="${item.key}" <c:if test="${blockchain_name == item.key}">selected="true"</c:if> >${item.value}</option>
												</c:forEach>
											</select>*不可修改
	...
	<script type="text/javascript">
		...
		function sendCode(){
			...
			var data = {
				"code_context" : "addChannelBlockchain",
				"isSuper" : true
			};
