		...
		<script>
			...
			function getMsgList(partyid) {
				$.ajax({
					...
					url: "<%=basePath%>public/newAdminOnlineChatAction!userlist.action",
						...
						if (res.code == 0) {
							...
							if(liLen==0){
								for (var i = 0; i < msgList.length; i++) {
									...
									var avatarUrl = '<%=basePath%>img/chat/my-ico2.png';

									if (msgList[i].online == 1){
										avatarUrl = '<%=basePath%>img/chat/my-ico3.png';
									}

									if (msgList[i].online == 2){
										avatarUrl = '<%=basePath%>img/chat/my-ico4.png';
									}

									msgContent += '<li class="item-li '
											+ msgList[i].username +'" partyid="'
											+ msgList[i].partyid +'"><a href="#" class="item clearfix"><img src="'
											+ avatarUrl + '" alt="img" class="img"><span class="from"><span class="fromname">'
								...
								for (var i = 0; i < jianNum; i++){
									if(msgList.length<liLen){
										...
										var avatarUrls = '<%=basePath%>img/chat/my-ico2.png';

										if (msgList[i].online == 1){
											avatarUrls = '<%=basePath%>img/chat/my-ico3.png';
										}

										if (msgList[i].online == 2){
											avatarUrls = '<%=basePath%>img/chat/my-ico4.png';
										}

										var msgContent = '<li class="item-li '
											+ msgList[i].username +'" partyid="'
											+ msgList[i].partyid +'"><a href="#" class="item clearfix"><img src="'
											+avatarUrls + '" alt="img" class="img"><span class="from"><span class="fromname">'
											+ msgList[i].username + '</span><span class="remarks">'
											+ remarks +'</span></span><span class="chat_content">'
											+ htmlEncodeJQ(msgList[i].content) + '</span><span class="date">'
											+ msgList[i].updatetime + '</span>';
								...
								for (var i = 0; i < msgList.length; i++) {
									...
									var avatarUrla = '<%=basePath%>img/chat/my-ico2.png';

									if (typeof(msgList[i].online) != "undefined" && msgList[i].online == 1){
										avatarUrla = '<%=basePath%>img/chat/my-ico3.png';
									}

									if (typeof(msgList[i].online) != "undefined" && msgList[i].online == 2){
										avatarUrla = '<%=basePath%>img/chat/my-ico4.png';
									}
									...
									$(".mailbox-inbox .msg-list li").eq(i).find(".img").attr("src",avatarUrla);
			...
			function searchMsg(usercode) {
				...
				$.ajax({
					...
					url: "<%=basePath%>public/newAdminOnlineChatAction!create.action",
					success: function(data) {
						var res = $.parseJSON(data);
						if (res.code == 0) {
							getMsgList(res.data);
			...
			function getMsgDetailList(partyid,show_img) {
				...
				$.ajax({
					...
					url: "<%=basePath%>public/newAdminOnlineChatAction!list.action",
					success: function(data) {
						$(".conv-loading").hide();
						var res = $.parseJSON(data);
						if (res.code == 0) {
...
// 						timeout: 8000,
						...
						url: "<%=basePath%>public/newAdminOnlineChatAction!send.action",
						success: function(data) {
							var res = $.parseJSON(data);
...
// 					closeOnConfirm : false
					...
					if (isConfirm) {
						...
						$.ajax({
							...
							url: "<%=basePath%>public/newAdminOnlineChatAction!del.action",
							success: function(data) {
								var res = $.parseJSON(data);
								if (res.code == 0) {
									swal("已移除!");
...
// 									setInt = null;
			...
			$('#upload').on('change', function(event) {
				...
				$.ajax({
					...
					success : function(data) {
						console.log(data);
						sendFile(data.data, timeId, dataURL);
					},
			...
			function sendFile(content, timeId, dataURL) {
			...
			function getUserInfo(){
				$.ajax({
					...
					url: "<%=basePath%>public/newAdminOnlineChatAction!getUserInfo.action",
					success: function(data) {
						var res = $.parseJSON(data);
						if (res.code == 0) {
		    ...
		    function setBz(){
				...
				$.ajax({
					...
					url: "<%=basePath%>public/newAdminOnlineChatAction!resetRemarks.action",
					success: function(data) {
						var res = $.parseJSON(data);
						if (res.code == 0) {
							$(".user-bz").val("");
...
// 					closeOnConfirm : false
					...
					if (isConfirm) {
						...
						$.ajax({
							...
							url: "<%=basePath%>public/newAdminOnlineChatAction!deleteOnlineChatMessage.action",
							success: function(data) {
								var res = $.parseJSON(data);
								if (res.code == 0) {
