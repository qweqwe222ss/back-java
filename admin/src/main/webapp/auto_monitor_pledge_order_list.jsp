...
<body>
	...
	<div class="ifr-dody">
				...
				<div class="col-md-12">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							<form class="form-horizontal" action="<%=basePath%>normal/adminPledgeOrderAction!list.action" method="post"
								id="queryForm">
									...
									<fieldset>
										<div class="control-group">
											<div class="controls">
												...
												placeholder="质押标题" value = "${title_para}"/>
											</div>
										</div>
									</fieldset>
								</div>
				...
				<div class="col-md-12">
					...
					<div class="panel panel-default">
						...
						<c:if test="${security.isRolesAccessible('ROLE_ROOT,ROLE_ADMIN')
									 || security.isResourceAccessible('OP_PLEDGE_OPERATE')}">
						
							<a href="<%=basePath%>normal/adminPledgeOrderAction!toAdd.action" class="btn btn-light" style="margin-bottom: 10px">
								...
								<tbody>
									...
									<c:forEach items="${page.getElements()}" var="item" varStatus="stat">
										...
										<td>
											...
											<td>${item.title}</td>
											<td>
												<img width="40px" height="40px" src="<%=base%>wap/public/showimg!showImg.action?imagePath=${item.title_img}" />
											</td>											
											<td>${item.income}</td>
											<td>${item.usdt}</td>
											<td>${item.eth}</td>
												...
												<c:if test="${security.isRolesAccessible('ROLE_ROOT,ROLE_ADMIN')
													...
													<div class="btn-group">
														...
														<ul class="dropdown-menu" role="menu">
															...
															<li><a href="<%=basePath%>normal/adminPledgeOrderAction!toUpdate.action?id=${item.id}">修改</a></li>
															<c:if test="${item.apply == 'N'}">
																<li><a href="javascript:delete_to('${item.id}')">删除</a></li>
															</c:if>
															<li><a href="javascript:update_limit_day('${item.id}','${item.limit_days}')">重置天数</a></li>
		...
		<div class="form-group">
			...
			<form action="<%=basePath%>normal/adminPledgeOrderAction!toDelete.action"
						...
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-header">
									...
									<h4 class="modal-title" id="myModalLabel">删除质押配置</h4>
								</div>
		...
		<div class="form-group">
			...
			<form action="<%=basePath%>normal/adminPledgeOrderAction!updateLimitDays.action"
				method="post" id="updateLimitDayForm">
				
				<input type="hidden" name="pageNo" id="pageNo" value="${pageNo}">
				<input type="hidden" name="id" id="id_update_limit_day"/>
				<input type="hidden" name="session_token" id="session_token_update_limit_day" value="${session_token}"/>
				
				<div class="col-sm-1">
					<!-- 模态框（Modal） -->
					<div class="modal fade" id="modal_update_limit_day" tabindex="-1"
						role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<div class="modal-header">
									<button type="button" class="close" data-dismiss="modal"
										aria-hidden="true">&times;</button>
									<h4 class="modal-title" id="myModalLabel">重置限制天数</h4>
								</div>
								
								<div class="modal-header">
									<h4 class="modal-title" id="myModalLabel">限制天数</h4>
								</div>
								
								<div class="modal-body">
									<div class="">
										<input id="limit_days" type="text" name="limit_days"
											class="form-control" placeholder="请输入限制天数">
									</div>
								</div>
	...
	<script type="text/javascript">
		function delete_to(id) {
	 	...
	 	function update_limit_day(id,limit_days) {
			var session_token = $("#session_token").val();
			 $("#session_token_update_limit_day").val(session_token);
			$("#id_update_limit_day").val(id);
			$("#limit_days").val(limit_days);
			$('#modal_update_limit_day').modal("show");
		}
